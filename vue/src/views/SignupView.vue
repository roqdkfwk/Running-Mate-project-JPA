...
const checkId = function () {
  ...
  // 아이디 중복확인 결과
  idChecked.value = userStore.checkId(form.value.userId)
}
...
const checkNick = function () {
  // 닉네임 입력값 검증: 알파벳과 숫자만 허용
  if (!/^[a-zA-Z0-9]+$/.test(form.value.userId)) {
    alert("닉네임은 알파벳과 숫자만 입력 가능합니다.");
    nickChecked.value = false; // 중복 확인 상태 초기화
    return; // 서버 요청 중단
  }

  // 닉네임 중복확인 결과
  nickChecked.value = userStore.checkNick(form.value.userId)
}
...
const submitForm = function () {
  ...
  if (isPasswordMatch.value) {
    ...
    userStore.signup(newUser) // 회원가입 요청
  } else {
